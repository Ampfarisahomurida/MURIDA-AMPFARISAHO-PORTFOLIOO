...
const DATA_DIR = path.join(__dirname, 'MURIDA-AMPFARISAHO-PORTFOLIOO-main', 'data');
...
const server = http.createServer((req, res) => {
    ...
    if (req.url === '/api/chat' && req.method === 'POST') {
        ...
        req.on('end', async () => {
            try {
                ...
                // Ensure fetch is available before provider calls
                try { await ensureFetch(); } catch (e) { console.warn('ensureFetch failed:', e && e.message); }
    ...
    // Serve admin page
    if (req.url === '/admin') {
        const adminPath = path.join(__dirname, 'MURIDA-AMPFARISAHO-PORTFOLIOO-main', 'admin.html');
    ...
    // Serve static files
    let filePath = path.join(__dirname, req.url === '/' ? 'MURIDA-AMPFARISAHO-PORTFOLIOO-main/index.html' : req.url);
    // If the path looks like a nested file, resolve inside folder
    if (!fs.existsSync(filePath)) {
        filePath = path.join(__dirname, 'MURIDA-AMPFARISAHO-PORTFOLIOO-main', req.url === '/' ? 'index.html' : req.url);
    }
    ...
    fs.readFile(filePath, (err, content) => {
        if (err) {
            if (err.code === 'ENOENT') {
                ...
                fs.readFile(path.join(__dirname, 'MURIDA-AMPFARISAHO-PORTFOLIOO-main', 'index.html'), (err, content) => {
                    res.writeHead(200, { 'Content-Type': mimeTypes['.html'] });
